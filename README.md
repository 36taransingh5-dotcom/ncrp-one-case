...
Verify a fresh deployment with `GET /api/health` and the operator page `/integrations`. Production should return `{ "status": "ok", "backend": "supabase" }` and, when the worker secret is set, `"integrations": { "mode": "http" }`. Email is LIVE when `RESEND_API_KEY` is set. Citizen inboxes require `RESEND_FROM` on a verified Resend domain; the default test sender only delivers to the Resend account owner. Reporting and beneficiary-bank jobs enqueue from the app even if migration `012` has not been applied yet.
