...
Verify a fresh deployment with `GET /api/health`; production should return `{ "status": "ok", "backend": "supabase" }` and, when the worker secret is set, `"integrations": { "mode": "http" }`. Reporting and beneficiary-bank jobs enqueue from the app even if migration `012` has not been applied yet.
