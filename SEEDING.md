...
To provision the optional mentor showcase, also set `NCRP_SHOWCASE_CITIZEN_EMAIL` and `NCRP_SHOWCASE_CITIZEN_PASSWORD`, then run `npm run seed:showcase`. This creates or refreshes only synthetic case `NCRP-26-926184`, uploads its synthetic evidence object to the private bucket, and preserves the golden-path case and arbitrary citizen-created cases.
