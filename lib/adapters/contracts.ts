...
export interface PoliceAdapter {
  assignCyberCell(caseId: string): Promise<{ assignmentReference: string }>;
  startFirReview(caseId: string): Promise<{ reviewReference: string }>;
  registerFir(caseId: string): Promise<{ firNumber: string }>;
}
