...
async function main() {
  ...
  const citizen = await ensureUser(
    demoIdentity.citizenEmail,
    demoIdentity.citizenPassword,
    "Asha Mehta (synthetic)",
  );
  const operator = await ensureUser(
    demoIdentity.operatorEmail,
    demoIdentity.operatorPassword,
  ...
  const { data: caseId, error: seedError } = await admin.rpc(
    "seed_round2_demo",
    {
  ...
  console.info(
    `Seeded ${caseId} with synthetic citizen ${demoIdentity.citizenEmail} and operator ${demoIdentity.operatorEmail}.`,
  );
}
